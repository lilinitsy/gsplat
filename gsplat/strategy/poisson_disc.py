...
from .ops import duplicate, duplicate_with_poisson, remove, reset_opa, split, split_with_poisson


@dataclass
class PoissonDiscStrategy(Strategy):
    ...
    Examples:
        ...
        >>> from gsplat import PoissonDisc, rasterization
        >>> params: Dict[str, torch.nn.Parameter] | torch.nn.ParameterDict = ...
        >>> optimizers: Dict[str, torch.optim.Optimizer] = ...
        >>> strategy = PoissonDisc()
    ...
    def _grow_gs(
        ...
        count = state["count"] # #times visible
        grads = state["grad2d"] / count.clamp_min(1) # 2D gradients, averaged
        device = grads.device
		...
		# The point of this is that if a Gaussian is small, it can't be further split, so densification 
        # has to be done by placing more Gaussians.

        # first duplicate
        if n_dupli > 0:
            #duplicate(params=params, optimizers=optimizers, state=state, mask=is_dupli)
            duplicate_with_poisson(params = params, optimizers = optimizers, state = state, mask = is_dupli,
                                    min_dist_factor = 1, k_neighbours = 20, max_attempts = 30)
